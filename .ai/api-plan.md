...
```json
{
  "title": "Updated Vegan Chocolate Cake",
  "ingredients": "2 cups flour, 1 cup sugar...",
  "instructions": "Preheat oven to 350°F...",
  "updated_at": "2024-01-15T10:30:00Z"
}
...
**Field Details**:
- `title` (string, required): Recipe title (max 200 characters)
- `ingredients` (string, required): Recipe ingredients
- `instructions` (string, required): Recipe instructions
- Combined length of `ingredients` and `instructions` must not exceed 10,000 characters
- `updated_at` (string, required): Timestamp from the client's version for optimistic locking
...
- `404 Not Found`: Recipe not found or user doesn't own the recipe
- `400 Bad Request`: Invalid input data (validation errors)
- `409 Conflict`: Recipe was modified by another session (optimistic locking failed)
  ```json
  {
    "error": "Conflict",
    "message": "Recipe was modified by another session. Please refresh and try again.",
    "current_version": {
